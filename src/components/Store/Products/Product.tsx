...
export default function Product({ products }: ProductProps) {
  ...
  const handleSaveProductToLocal = (product: ProductType) => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[20px] ">
        {paginatedProducts.map((product) => (
          ...
          >
            ...
            <div className="p-4">
              <h3 className="text-sm font-medium text-gray-900 line-clamp-2 mb-1"><Link href={`store/product/${product.id}`} > <a onClick={() => handleSaveProductToLocal(product)}>{product.title}</a> </Link></h3>

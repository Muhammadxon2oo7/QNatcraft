...
export default function Filter({ onFilterChange }: FilterProps) {
  ...
  const toggleOption = useCallback(
    (sectionId: string, optionId: string) => {
      ...
      setSections((updatedSections) => {
        const selectedCategories = updatedSections
  ...
  const toggleDiscountFilter = () => {
    setSortByDiscount((prev) => {
      const newSortByDiscount = !prev
      const selectedCategories = sections
        .flatMap((section) => section.options)
        .filter((option) => option.checked)
        .map((option) => option.id)
      onFilterChange(selectedCategories, newSortByDiscount)
      return newSortByDiscount
    })
  }

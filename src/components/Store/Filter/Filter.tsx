...
// import { ChevronDown, ChevronUp, Check, AlertCircle, Search, X, Heart } from "lucide-react"
...
//   onFilterChange: (selectedCategories: string[], sortByDiscount: boolean, showLiked: boolean, searchTerm?: string) => void
// }
...
//   const [showLikedOnly, setShowLikedOnly] = useState(false) // Yangi: Yoqtirganlarim filtri
...
//         onFilterChange(selectedCategories, sortByDiscount, showLikedOnly, searchTerm)
//         return newState
//       })
//     },
//     [onFilterChange, sortByDiscount, showLikedOnly, searchTerm]
//   )
...
//       onFilterChange(selectedCategories, newValue, showLikedOnly, searchTerm)
//       return newValue
//     })
//   }, [getSelectedCategories, onFilterChange, showLikedOnly, searchTerm])

//   const toggleLikedFilter = useCallback(() => {
//     setShowLikedOnly((prev) => {
//       const newValue = !prev
//       const selectedCategories = getSelectedCategories()
//       onFilterChange(selectedCategories, sortByDiscount, newValue, searchTerm)
//       return newValue
//     })
//   }, [getSelectedCategories, onFilterChange, sortByDiscount, searchTerm])
...
//       const selectedCategories = getSelectedCategories()
//       onFilterChange(selectedCategories, sortByDiscount, showLikedOnly, newSearchTerm)
//     },
//     [getSelectedCategories, onFilterChange, sortByDiscount, showLikedOnly]
//   )

//   const clearSearch = useCallback(() => {
//     setSearchTerm("")
//     const selectedCategories = getSelectedCategories()
//     onFilterChange(selectedCategories, sortByDiscount, showLikedOnly, "")
//   }, [getSelectedCategories, onFilterChange, sortByDiscount, showLikedOnly])
...
//       onFilterChange(selectedCategories, sortByDiscount, showLikedOnly, searchTerm)
//     },
//     [getSelectedCategories, onFilterChange, searchTerm, sortByDiscount, showLikedOnly]
//   )
...
//                 showLikedOnly ? "text-primary" : "text-gray-800"
//               }`}
//               aria-pressed={showLikedOnly}
//             >
//               Yoqtirganlarim
//               <Heart size={20} className={showLikedOnly ? "text-red-500 fill-red-500" : "text-gray-600"} />
//             </button>
//           </div>
...
import { motion, AnimatePresence } from "framer-motion"
import { Check, AlertCircle, Search, X, Heart } from "lucide-react"
import { Card } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import { Alert, AlertDescription } from "@/components/ui/alert"
import { Input } from "@/components/ui/input"
    ...
    if (categories) {
      ...
      const fetchCategories = async () => {
        try {
          ...
          setFilterCategories(data.map((category: any) => ({
            id: String(category.id),
            label: category.name,
            count: category.product_count || 0,
          })))
...
const handleFilterChange = useCallback(() => {
  ...
  return (
    <Card className="p-4 bg-white rounded-xl shadow-sm">
      <div className="flex justify-between items-center mb-4 pb-4 border-b md:border-none">
        <h2 className="text-lg font-semibold hidden md:block">Filter</h2>
        <Button variant="ghost" onClick={clearAll} className="text-red-600 text-sm">
          Tozalash
        </Button>
      </div>
      ...
      {error && (
        <Alert variant="destructive" className="mb-4">
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>{error}</AlertDescription>
          <Button variant="outline" size="sm" onClick={retryFetch} className="mt-2 text-xs">
            Qayta urinib ko'rish
          </Button>
        </Alert>
      )}
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            <Input
              placeholder="Qidirish..."
              className={`pr-10 text-sm ${isFocused ? 'ring-2 ring-primary/30' : ''}`}
              value={searchTerm}
              onChange={handleSearchChange}
              onFocus={() => setIsFocused(true)}
              onBlur={() => setIsFocused(false)}
            />
            {searchTerm && (
              <button className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" onClick={clearSearch}>
                <X size={16} />
              </button>
            )}
            <Search size={16} className="absolute right-10 top-1/2 -translate-y-1/2 text-gray-400" />
          </div>
          ...
          <div className="flex md:hidden flex-col gap-3">
            <Select value={selectedCategory} onValueChange={setSelectedCategory}>
              ...
              <SelectContent>
                <SelectItem value="none" className="text-sm">Barchasi</SelectItem>
          ...
          <div className="hidden md:block space-y-4">
            ...
            <div>
              <h3 className="text-sm font-medium mb-2">Hunarmandchilik turi</h3>
                  ...
                  <div key={category.id} className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className={`h-4 w-4 rounded border flex-center transition ${
                    ...
                    <span className={selectedCategory === category.id ? "text-primary" : "text-gray-600"}>
                      {category.count}
                    </span>
                  </div>
                ))}

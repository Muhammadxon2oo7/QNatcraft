    ...
    <div className="flex justify-center gap-4 mb-4">
      {languages.map(({ code, label }) => (
        <Button
          ...
          variant="outline"
          size="sm"
          className={`w-20 h-[40px] rounded-[16px] ${locale === code ? 'bg-primary text-white border-primary' : 'border-primary text-primary'} ${isPending ? 'opacity-50 cursor-not-allowed' : ''}`}
          onClick={() => onChange(code)}
          disabled={isPending}
        >
          {label}

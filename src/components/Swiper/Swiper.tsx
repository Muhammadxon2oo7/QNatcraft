...
const CraftsmanCard = ({
  ...
  return (
    <div
      className={`rounded-3xl p-4 sm:p-5 md:p-6 transition-all duration-500 ease-in-out w-full ${cardClass} shadow-md flex flex-col h-full`}
      aria-hidden={!isVisible}
    >
      <div className="flex flex-col-reverse md:flex-row gap-4 md:gap-6 flex-1">
        {/* Matnli qism */}
        <div className="flex-1 flex flex-col justify-between">
          <div>
            <Badge
            ...
            <h3 className="text-[clamp(16px,2vw,18px)] font-semibold text-[#242b3a] line-clamp-2">
              {craftsman.user_first_name || t("unknown")}
            </h3>
            <p className="text-gray-500 text-[clamp(12px,1.5vw,14px)] mb-4 md:mb-6 line-clamp-3">
              {craftsman.bio || t("noBio")}
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 mb-4 md:mb-6">
              <div className="flex items-center gap-2 md:gap-3">
                <div className="rounded-full p-1.5 bg-white">
                  <Litsense aria-hidden="true" />
                </div>
                <p className="text-[clamp(12px,1.5vw,14px)] font-semibold text-[#242b3a]">
                  {craftsman.experience} {t("yearsExperience")}
                </p>
              </div>
              <div className="flex items-center gap-2 md:gap-3">
              ...
              {craftsman.award && (
                <div className="flex items-center gap-2 md:gap-3 col-span-1 md:col-span-2">
                  ...
                  <p className="text-[clamp(12px,1.5vw,14px)] font-semibold text-[#242b3a] line-clamp-1">
                    {craftsman.award}
                  </p>
                </div>
              )}
        ...
        <div className="flex-shrink-0 w-full md:w-48 lg:w-60">
          <Image
            ...
            height={200}
            sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
            className="rounded-2xl object-cover w-full h-[clamp(160px,40vw,200px)] md:h-[clamp(180px,30vw,240px)]"
            priority={isActive}
            loading={isActive ? "eager" : "lazy"}
          />
        </div>
...
const CustomSwiper = () => {
  ...
  const fetchProfessions = useCallback(async () => {
    try {
      ...
      const response = await fetch("https://qqrnatcraft.uz/accounts/professions/", { signal: controller.signal });
  ...
  const fetchCraftsmen = useCallback(async () => {
    ...
    try {
      ...
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 5000);
      const response = await fetch("https://qqrnatcraft.uz/accounts/profiles/", { signal: controller.signal });
      clearTimeout(timeoutId);
  ...
  // Kasb nomini olish
  const getProfessionName = useMemo(
    () => (professionId: number | null | undefined) => {
      if (!professionId) return t("unknown");
      const profession = professions.find((p) => p.id === professionId);
      return profession?.name || t("unknown");
  ...
  if (isLoading) {
    return (
      <div className="text-center py-10">
        <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-primary mx-auto"></div>
        <p className="mt-2 text-[clamp(14px,1.5vw,16px)]">{t("loading")}</p>
      </div>
    );
  }
  ...
  if (craftsmen.length < 2) {
    return (
      <div className="text-center py-10">
        <p className="text-[clamp(14px,1.5vw,16px)]">
          {t("notEnoughCraftsmen")} (Found: {craftsmen.length})
        </p>
      </div>
    );

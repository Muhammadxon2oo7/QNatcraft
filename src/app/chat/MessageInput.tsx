...
const MessageInput: React.FC<MessageInputProps> = ({
  ...
  const handleSendMessage = useCallback(async () => {
    if (!wsConnection || (!debouncedInput.trim() && !selectedImages.length && !recordedAudio) || isSending) return;
    ...
    try {
      if (selectedImages.length > 0) {
        ...
        const token = await getToken();
        const formData = new FormData();
        formData.append("voice", recordedAudio, "audio.webm");
        if (replyingTo) formData.append("reply_to", replyingTo.id.toString());
  ...
  }, [chatId, wsConnection, debouncedInput, selectedImages, editingMessage, replyingTo, getToken, isSending, recordedAudio, scrollToBottom]);

  const toggleRecording = useCallback(async () => {
    if (isRecording) {
      mediaRecorderRef.current?.stop();
      setIsRecording(false);
      setRecordedAudio(null); // Yozib olingan ovoz o‘chiriladi
    } else {
      try {
        const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
        const recorder = new MediaRecorder(stream);
    ...
    <div className="p-3 sm:p-4 border-t bg-white relative shadow-sm">
      ...
      <div className="flex items-center bg-gray-100 rounded-full p-2 space-x-2 relative">
        ...
        <Button
          ...
          onClick={handleSendMessage}
          aria-label="Send message"
          disabled={isSending || (!debouncedInput.trim() && !selectedImages.length && !recordedAudio)}
          className="hover:bg-blue-100"
        >
          <Send size={24} className="text-blue-500" />
        </Button>
        {showEmojiPicker && (
          <motion.div
            className="absolute bottom-16 left-0 z-100"
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.3 }}
          >
